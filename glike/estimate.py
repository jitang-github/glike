...
class Search():
  def __init__(self, x0, bounds = None, precision = 0.05, lrs = None):
    ...
    if lrs is None:
      ...
      # Restore per-parameter learning rates (used when resuming a previous run).
      self.lrs = {name: lrs[name] for name in self.names}
...
def maximize(fun, x0, bounds = None, precision = 0.05, epochs = 20, verbose = False,
             log_file = None, start_epoch = 0, lrs0 = None):
  ...
  # log_file: optional path to a structured trajectory log (opened in append mode).
  # start_epoch: integer epoch index to use for the first iteration. Used by
  #              resume to make epoch numbering continuous across sessions.
  # lrs0: optional {name: lr} dict used to restore Search learning rates
  #       when resuming.

  search = Search(x0, bounds = bounds, precision = precision, lrs = lrs0)
  names = list(x0.keys())

  log_f = open(log_file, "a") if log_file is not None else None
  def _log(msg = ""):
  ...
  try:
    _log("# === session ===")
    _log("# epochs requested: " + str(epochs))
    _log("# start_epoch: " + str(start_epoch))
    _log("# x0: " + str(x0))
    if lrs0 is not None:
      _log("# x0 lrs: " + str(dict(lrs0)))

    y0 = fun(**x0)
    print(str(x0) + " " + str(y0), flush = True)
    _log("# initial: " + str(x0) + " " + str(y0))

    xs = []
    ys = []
    stopped_early = False
    last_epoch_num = start_epoch - 1
    for epoch_num in range(start_epoch, start_epoch + epochs):
      _log("")
      _log("# epoch " + str(epoch_num))
      for name in names:
        x = search.get()
        y = fun(**x)
        x_up = search.up(name)
        ...
        _log("## probe param: " + name)
        _log("x_up:   " + str(x_up) + " " + str(y_up))
        _log("x:      " + str(x) + " " + str(y))
        _log("x_down: " + str(x_down) + " " + str(y_down))
      ...
      _log("# epoch " + str(epoch_num) + " end: " + str(x) + " " + str(y))
      _log("# epoch " + str(epoch_num) + " lrs: " + str(dict(search.lrs)))
    ...
    if stopped_early:
      _log("# status: stopped early at epoch " + str(last_epoch_num) +
           " (< " + str(epochs) + " requested)")
    else:
      _log("# status: completed (" + str(epochs) + "/" + str(epochs) + " epochs)")

    idx = ys.index(max(ys))
    x, y = xs[idx], ys[idx]
    _log("# best: epoch " + str(start_epoch + idx) + " " + str(x) + " " + str(y))
    return x, y
  finally:
    if log_f is not None:
      log_f.close()
